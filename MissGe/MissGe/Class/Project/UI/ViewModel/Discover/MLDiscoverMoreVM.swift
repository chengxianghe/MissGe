...
class MLDiscoverMoreVM: NSObject {
    ...
    func SetConfig() {
        ...
        modelObserable.asObservable().bind(to: tableView.rx.items) { tableView, row, model in
            let cell = tableView.dequeueReusableCell(withIdentifier: "MLDiscoverCell") as? MLDiscoverCell
        ...
        requestNewDataCommond.subscribe { (event: Event<Bool>) in
            if event.element! {
                ...
                self.provider
                    .rx
                    .request(.DiscoverMore(page: page))
