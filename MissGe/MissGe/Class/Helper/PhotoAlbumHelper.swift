...
typealias PhotoSaveBlock = (_ result: PhotoAlbumHelperResult, _ error: NSError?) -> ();
...
class PhotoAlbumHelper : NSObject {
     ...
     */
    class func saveImageToAlbum(_ image: UIImage, albumName: String = "", completion: ((_ result: PhotoAlbumHelperResult, _ error: NSError?) -> ())?) {
    ...
    // OC调用
    class func saveVideoToAlbum(_ videoFileURL: URL, albumName: String = "", completion: ((_ result: Bool, _ error: NSError?) -> ())?) {
    ...
    class func saveVideo(_ videoFileURL: URL, to albumName: String = "", completion: ((_ result: PhotoAlbumHelperResult, _ error: NSError?) -> ())?) {
        //权限验证
        self.requestAuthorization { (status) in
            if status != PHAuthorizationStatus.authorized {
                completion?(.denied, nil)
